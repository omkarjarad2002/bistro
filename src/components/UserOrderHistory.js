    ...
    <div className="fooditems__container text-align-center pt-5 pb-5 container ">
      ...
      <div className="card   d-flex mt-4 d-inline-flex ms-4 getabout2">
        <div className="card-body"> 
          <p className="card-title"><Link to={`/totalinfo/${order?.restaurantID}`}>Restaurant ID : {order?.restaurantID}</Link></p>   
          ...
          <p className="card-title" style={{color:"red"}}>Order Status : {order?.orderStatus}</p>
          <p className="card-title" style={{color:"green"}}>Note : {order?.reason}</p>
        </div>
      </div>
      ))}
    </div>
